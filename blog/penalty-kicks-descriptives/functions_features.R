...
add_features <- function(data) {
  data |>
    ...
    dplyr::ungroup() |>
    dplyr::left_join(
      get_df_most_common_position(),
      by = dplyr::join_by(taker_id == player_id)
    ) |>
    dplyr::mutate(
      most_common_start_position_binned = dplyr::case_when(
        is.na(most_common_start_position) ~ NA_character_,
        TRUE ~ substr(most_common_start_position, 1, 1)
      )
    )
}
