...
Conditional logistic regression[^1] is a classic tool in an epidemiologist's toolkit. It's usually used to analyse matched case-control studies. However, popular guidance has advocated to 'break' the matching and analyse such studies using a ordinary logistic model for small efficiency gains. In this blog posts I want to demonstrate the superiority of conditional logistic regression in well-designed matched case-control studies of omic data that contain a substantial amount of technical variation.
...
Let's consider the following hypothetical omics study. Cases were 1:1 matched to controls on important covariates. Crucially, the case-control pairs were then block randomized over the analytical assay. In other words, the matched pairs (blocks) were randomized over the plates and batches and within the block we randomize whether the position of the case and control. In visual form:
...
```
This design ensures that the case and its matched control are processed consecutively and as such are processed under highly similar laboratory conditions (same plate, often adjacent wells, same batch, similar time).

This elegant formulation eliminates the stratum-specific intercept terms, effectively controlling for all measured and unmeasured factors that are constant within strata.
...
[^2]: Interestingly, CLR models can sometimes be estimated using generalized linear mixed models software by specifying a particular random intercepts structure, although clogit is the standard. See for example the [Metafor documentation on fitting CLR with GLMM](https://www.metafor-project.org/doku.php/tips:clogit_paired_binary_data)).
...
Conditional logistic regression bypasses this problem. Because it conditions on the matched pair (stratum), any technical factor that affects both the case and the control in that pair approximately equally gets cancelled out, just like the α 

The "unobserved stratum-level variables" that CLR adjusts away are precisely these shared technical artifacts within the pair.
  


# Caveats

Maybe conditional on total total amount of variation in study. I should run some simulations (shouldn't we all?)
